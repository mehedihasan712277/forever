...
const UpdateAddress = () => {
    const { user, changeY, y } = useContext(AuthContext);
    ...
    const handleEdit = (event) => {
        ...
        axios.put(`https://server-forever.vercel.app/userAddress/${user.uid}`, address)
            .then(res => {
                Swal.fire({
                    title: "Success",
        ...
        <>
            <div>
                <form className='flex flex-col gap-2 items-center pt-6 pb-20' onSubmit={handleEdit}>
                    <p>Edit address here</p>
                    {/* <p className='font-semibold text-2xl mb-6'>Enter your address here</p> */}
                    <input className='bg-blue-100 p-2 rounded-sm w-64 mm:w-72' defaultValue={data?.house} name="house" type="text" placeholder='House & local area name' required />
                    <input className='bg-blue-100 p-2 rounded-sm w-64 mm:w-72' defaultValue={data?.policeStation} name="policeStation" type="text" placeholder='Police Station' required />
                    ...
                    <input className='bg-blue-100 p-2 rounded-sm w-64 mm:w-72' defaultValue={data?.phone} name="phone" type="number" placeholder='Phone number' required />
                    <button type='submit' className='btn btn-neutral btn-outline'>Save</button>
                </form>
            </div>
        </>
    )
